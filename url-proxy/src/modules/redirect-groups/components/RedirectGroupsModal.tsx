...
export type RedirectGroupModalProps = {
  entity?: RedirectGroup;
  onCreate: (data: RedirectGroup) => void;
  onUpdate: (id: string, data: RedirectGroup) => void;
} & Omit<BaseModalProps, "children">;
const RedirectGroupModal = ({
  entity,
  onCreate,
  onUpdate,
  ...baseProps
}: RedirectGroupModalProps) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
    setValue,
  } = useForm<RedirectGroup>();
  ...
  const title = useMemo(() => {
    return entity ? "Update redirect group" : "Create redirect group";
  }, [entity]);
...
export default RedirectGroupModal;
